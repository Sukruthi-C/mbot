...
def main():
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            calibration_successful, mtx, dist,rvecs,tvecs = calibrate_camera(frame)
            if calibration_successful:
                ...
                for i in range(len(rvecs)):
                    ...
                    R_mtx, _ = cv2.Rodrigues(rvecs[i])
                    extrinsic_matrix = np.hstack((R_mtx, tvecs[i]))

                    print(f"Extrinsic Matrix for Image {i}:", extrinsic_matrix)
