...
// serial_mbot_motor_vel_t my_cmd;

mbot_ctlr_cfg_t mbot;
serial_mbot_motor_pwm_t my_cmd;
    ...
    if (global_comms_status == COMMS_OK)
    {
        if(drive_mode == MODE_MOTOR_VEL_OL){
            ...
            if(MBOT_DRIVE_TYPE == OMNI_120_DRIVE){
                ...
                // float vL = rc_filter_march(&filter_L, mbot_motor_vel_cmd.velocity[params.mot_left] - mbot_motor_vel.velocity[params.mot_left]);
                // float vR = rc_filter_march(&filter_R, mbot_motor_vel_cmd.velocity[params.mot_right] - mbot_motor_vel.velocity[params.mot_right]);
                ...
                if (!bool_first)
                {
                //     float r = mbot_motor_pwm_cmd.pwm[params.mot_right];
                //     float l = mbot_motor_pwm_cmd.pwm[params.mot_left];
                       mbot_motor_vel_ctlr(mbot_motor_vel_cmd,mbot_motor_vel,&my_cmd,mbot_motor_pwm_cmd.pwm[params.mot_right],mbot_motor_pwm_cmd.pwm[params.mot_left]);
                //     mbot_motor_pwm_cmd.pwm[params.mot_left] = my_cmd.pwm[1];
                //     mbot_motor_pwm_cmd.pwm[params.mot_right] = my_cmd.pwm[0];

                }
                mbot_motor_pwm_cmd.pwm[params.mot_right] = _calibrated_pwm_from_vel_cmd(vel_right_comp+my_cmd.pwm[0], params.mot_right);
                mbot_motor_pwm_cmd.pwm[params.mot_left] = _calibrated_pwm_from_vel_cmd(vel_left_comp+my_cmd.pwm[1], params.mot_left);
...
{   
    ...
    // rc_filter_first_order_lowpass(&lowpass_filter, dt, time_constant);
