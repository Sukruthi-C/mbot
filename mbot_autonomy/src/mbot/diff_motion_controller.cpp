...
private:
    float ppp[3] = {1.5, 2.5, 0.0}; //kp, ka, kb
    float iii[3] = {0.1, 0.15, 0.0};
    float ddd[3] = {0.1, 0.1, 0.0};
    float pfwd_prev = 0;
    float alpha_prev = 0;
    float d_end_crit = 0.01;
...
public:
    ...
    virtual mbot_lcm_msgs::twist2D_t get_command(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target) override
    {
        ...
        float dx = target.x - pose.x;
        float dy = target.y - pose.y;
        float pfwd = sqrt(dx * dx + dy * dy);
        float alpha = angle_diff(atan2(dy,dx), pose.theta);
        
        float dfwd = pfwd-pfwd_prev;
        float dalpha = alpha-alpha_prev;
        ...
        // If alpha is more than 25 degrees, turn in place and then go
        if (fabs(alpha) > M_PI/180*20)
        {
            pfwd = 0;
        }
        ...
        {
            pfwd *= 0.7- (fabs(alpha)/(M_PI/180*20))*0.5 + 0.3;
        }

        //p
        float fwd_vel = vel_sign *  ppp[0] * pfwd;
        float turn_vel = ppp[1] * alpha + ppp[2] * beta;

        //d
        fwd_vel += vel_sign * dfwd * ddd[0];
        turn_vel +=  ddd[1] * dalpha + ddd[2] * beta;
...
int main(int argc, char** argv)
{
    ...
    float prev_vx = 0;
    float prev_wz = 0;
    float max_accel = 0.05;
    float max_ang_accel = M_PI*2/3;
    ...
    while(true)
    {
    	...
    	if(controller.timesync_initialized()){
            ...
            if (cmd.vx -prev_vx> max_accel) cmd.vx = prev_vx + max_accel;
            else if (cmd.vx - prev_vx < -max_accel) cmd.vx = prev_vx - max_accel;
            //velocity control
            if (cmd.vx > max_speed) cmd.vx = max_speed;
            else if (cmd.vx < -max_speed) cmd.vx = -max_speed;

            //angular acceleration control
            if (cmd.wz- prev_wz> max_ang_vel) cmd.wz = prev_wz+max_ang_accel;
            else if (cmd.wz- prev_wz< -max_ang_vel)cmd.wz = prev_wz-max_ang_accel;
